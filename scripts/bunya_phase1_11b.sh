#!/bin/bash
#SBATCH --job-name=vlm_phase1_11b
...
#SBATCH --output=logs/phase1_11b_%j.out
...
echo "[phase1_11b] scratch=$SCRATCH_HF"
echo "[phase1_11b] copying llama-11b weights (~23 GB) ..."
mkdir -p "$SCRATCH_HF/hub"
rsync -aL --info=progress2 "$SRC_MODEL" "$SCRATCH_HF/hub/"
echo "[phase1_11b] copy done"

export HF_HOME=$SCRATCH_HF
export SAFETENSORS_FAST_GPU=1

python -u scripts/run_phase1.py --model llama_dev --quantization none
